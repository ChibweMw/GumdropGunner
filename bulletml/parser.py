...
class Max_Hp(object):
    ...
    def __getstate__(self):
        return dict(max_hp=self.max_hp)

    def __setstate__(self, state):
        self.__init__(state["max_hp"])
...
class BulletDef(object):
    ...
    def __getstate__(self):
        ...
        if self.max_hp:
            state.append(("max_hp", self.max_hp))
...
class FireDef(object):
    ...
    def __init__(self, bullet, direction=None, speed=None, offset=None,
                 tags=(), appearance=None, max_hp=None):
        self.bullet = bullet
        self.direction = direction
        self.speed = speed
        self.offset = offset
    ...
    def FromXML(cls, doc, element):
        ...
        for subelem in element.getchildren():
            ...
            if tag == "direction":
                ...
                max_hp = subelem.text
        try:
            fire = cls(bullet, direction, speed, offset, tags, appearance, max_hp)
        except UnboundLocalError as exc:
            raise ParseError(str(exc))
        else:
    ...
    def __call__(self, owner, action, params, rank, created):
        direction, speed, tags, appearance, max_hp, actions = self.bullet(
        ...
        bullet = owner.__class__(
            ...
            appearance=appearance, max_hp=max_hp, tags=tags)
